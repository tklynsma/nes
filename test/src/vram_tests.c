...
char *test_3F00_3F1F(void) {
    ...
    vrm_write(0x3F00, 0x44);
    for (int i = 0; i < 8; i++) {
        ASSERT("Mirror 0x3F00-0x3F1F", vrm_read(0x3F00 + i * 0x20) == 0x44);
        ASSERT("Mirror 0x3F00-0x3F1F", vrm_read(0x7F00 + i * 0x20) == 0x44);
        ASSERT("Mirror 0x3F00-0x3F1F", vrm_read(0xBF00 + i * 0x20) == 0x44);
        ASSERT("Mirror 0x3F00-0x3F1F", vrm_read(0xFF00 + i * 0x20) == 0x44);
    }
